...
1. **Registration** — a teacher fills a form → we save them as a row in the `users` table (password scrambled, not plain text). Students never log in — they're just attendance records, not users.
...
3. **Access Control** — before showing a page, we check the wristband: Are you logged in? Are you a teacher or an admin? Wrong wristband = redirected away. Admins get one extra power: deleting accounts.
...
Only teachers and admins are users. Students don't log in — they belong in a separate `students` table (whoever's building the attendance feature owns that) and just get marked present/absent.

**Making the first admin:** registration always creates teachers (otherwise anyone could self-promote to admin). So register yourself normally, then run this once in Workbench:
...
**Logic:** Bootstrap classes (`card`, `form-control`, `btn`) do all the styling. `required` and `minlength` make the browser reject empty/short input before it even reaches the server — free validation. The `alert-danger` block only appears if the server sends back an error message. Notice there's **no role dropdown** — everyone who registers becomes a teacher automatically; admins are promoted via SQL (Step 0b) or by another admin. Never let a form decide who gets power.
...
app.post('/register', (req, res) => {
  ...
  bcrypt.hash(password, 10, (err, hashedPassword) => {
    // role is NOT taken from the form — everyone starts as a teacher
    const sql = 'INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, "teacher")';
...
app.post('/login', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (err, results) => {
    ...
    bcrypt.compare(password, user.password, (err, match) => {
      ...
      // Step 4: everyone lands on the dashboard (admins get extra menu items there)
...
```js
// Any logged-in user (teacher or admin)
...
app.get('/admin/users', requireLogin, requireRole('admin'), (req, res) => {
  db.query('SELECT id, name, email, role FROM users', (err, users) => {
    res.render('manageUsers', { user: req.session.user, users });
  });
});
```

**Logic:** the request walks through the gates left to right. Not logged in? Gate 1 bounces you to `/login`. Logged in as a teacher trying an admin page? Gate 2 shows a 403 "forbidden" page. Only if both gates say `next()` does the actual page render. This means even if a teacher *types the admin URL directly*, they still can't get in — hiding a link is not security, the gate is.

You can also show/hide things inside a page by role — e.g. on the dashboard:
...
## Step 4b — Admin: delete accounts

### The page (`views/manageUsers.ejs`) — a Bootstrap table with a delete button per row
...
```js
app.post('/admin/users/delete/:id', requireLogin, requireRole('admin'), (req, res) => {
  // safety: an admin can never delete their own account
...
**Logic, step by step:**
1. Delete is a **POST via a tiny form**, not a link — links (GET) can be triggered by accident (browser prefetch, someone sharing a URL). Actions that destroy data should always be POST.
2. `confirm(...)` pops up a browser "Are you sure?" box — one free line, no CSS, prevents fat-finger disasters.
3. The route wears **both gates**, so a teacher POSTing the URL directly still gets 403. Never rely on the button being hidden.
4. Self-delete is blocked twice: the button doesn't render for your own row, *and* the route checks again. The server-side check is the real one — anything in the browser can be bypassed.
5. `:id` in the route is a placeholder — `/admin/users/delete/7` means `req.params.id` is `7`.
...
3. Step 2 — register two test teachers, confirm rows appear in Workbench (passwords should look like `$2b$10$...`), promote one to admin via the UPDATE query → **commit**
4. Step 3 — log in as each, land on the dashboard → **commit**
5. Step 4 — as teacher, try `/admin/users` directly → should get 403 → **commit**
6. Step 4b — as admin, open Manage Users, delete the test teacher → row gone in Workbench → **commit**
7. Step 5 — logout, then try `/dashboard` → should bounce to login → **commit, push, open pull request, merge into main**

## Test checklist

- [ ] Register with an email that already exists → friendly error, no crash
- [ ] New registrations are always role `teacher` (no way to self-register as admin)
- [ ] Password in Workbench is hashed, not readable
- [ ] Wrong password → "Invalid email or password"
- [ ] Teacher typing `/admin/users` directly → 403 page
- [ ] Admin can delete a teacher; deleted teacher can no longer log in
- [ ] Admin cannot delete their own account (no button, and direct POST is blocked)
